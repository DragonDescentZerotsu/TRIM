...
Input 1. Neighbor similarities and per-neighbor comparison notes
"""
...
Input 2. Final prediction label
{{LOCAL_PREDICTION_SEMANTICS}}

Hard requirements:
1. Use only the listed neighbor similarities, the per-neighbor comparison notes, and the provided final prediction label.
...
18. You may use your own general chemistry, medicinal chemistry, and ADMET knowledge to interpret how the supplied neighbor/query feature values and deltas affect the final judgment, including common thresholds, value ranges, and mechanisms when they are relevant. However, those explanations must be tied to feature values already present in that neighbor's supplied comparison note, and they must not introduce new molecular evidence or override the draft's label direction for that neighbor.
19. If a descriptor effect depends on baseline context, make that dependence clear, but you do not need to force repetitive phrases such as "in this comparison" or "at this baseline" into every sentence.
20. After covering all 6 neighbors, explain how the six neighbor-level comparisons combine into one final prediction.
21. Make sure the final prediction matches the provided label.
22. Do not invent new neighbors, new similarities, new molecular evidence, or new experimental facts.
23. Do not mention model internals, pairwise EBM, aggregation code, prompt instructions, or hidden reasoning process.
24. Keep the final reasoning faithful to the original draft direction while making the prose more natural, coherent, and scientist-like.
25. The final `reasoning` must read like direct scientific reasoning, not commentary about source materials. **Do not** say "draft", "note", "prompt", "input", "instruction", "contribution", "pair score", or similar metadata words in the final text.
26. Do not use metadata nouns such as "the model", "the notes", "the comparison", "the contribution", or an unspecified "score" as the subject or source of judgment. Make the molecule, neighbor, query, property, raw value, delta, or chemistry mechanism the subject instead.
27. Do not mention EBM numeric model scores, prediction probabilities, confidence values, predictive scores, overall scores, net scores, or any other classification-score values. Molecular descriptor values such as a supplied QED score may still be used when they appear in the supplied neighbor notes.
28. Do not write phrases such as "in this draft", "in this note", "the prompt provides", or "this contribution pushes toward". Translate those ideas into direct chemistry reasoning instead.
...
- Prefer interval-aware explanations when common chemistry or ADMET knowledge makes a value range relevant, while keeping the explanation tied to the supplied neighbor/query values and deltas
