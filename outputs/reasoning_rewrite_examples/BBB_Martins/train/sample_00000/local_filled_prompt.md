You are rewriting rough molecule local analog-comparison drafts into high-quality reasoning SFT data for molecule local analog-comparison task BBB_Martins where option (A) means does not cross the BBB and option (B) means crosses the BBB.
...
Input 3. Final local prediction label
option (B): crosses the BBB

Hard requirements:
1. Use only the task playbook, the listed neighbor similarities, the per-neighbor local drafts, and the provided local prediction label.
2. Cover all listed neighbors. Do not silently drop any neighbor.
3. Keep positive-neighbor and negative-neighbor evidence distinct in the reasoning.
4. Explain how the neighbor-level comparisons combine into one final local conclusion.
5. Make sure the final local conclusion matches the provided local prediction label.
6. Do not invent new neighbors, new similarities, or new molecular evidence.
7. Do not mention model internals, pairwise EBM, aggregation code, or prompt instructions.
...
```json
{
  ...
  "quality_check": {
    ...
    "final_label_matches_local_prediction": true or false,
