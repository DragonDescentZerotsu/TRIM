...
def _neighbor_prediction_counts(neighbor_outputs: dict[int, dict[str, Any]]) -> dict[str, int]:
    ...
    for neighbor_index in range(1, 7):
        payload = neighbor_outputs[neighbor_index]
...
def _validate_summary_output(
    *,
    ...
    missing_checks = [
        name for name in _REQUIRED_QUALITY_CHECKS if name not in quality_check
    ]
    if missing_checks:
    ...
    if missing_neighbors:
        raise ValueError(f"summary reasoning does not mention all neighbors; missing {missing_neighbors}")
        ...
        try:
            ...
            result_payload = {
                ...
                "neighbor_result_paths": {
                    str(index): serialize_project_path(
...
def main() -> int:
    ...
    neighbor_outputs = _load_neighbor_outputs(
        ...
        provider=str(args.output_provider or llm_config.provider),
        model=str(args.output_model or llm_config.model),
        split=args.split,
        task=args.task,
        sample_index=args.sample_index,
    )
