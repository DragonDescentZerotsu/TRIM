...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run hybrid global+local molecule-level evaluation.")
    parser.add_argument("--task", required=True)
    parser.add_argument("--split", default="test")
    parser.add_argument("--pos-bundle-path", required=True)
    parser.add_argument("--neg-bundle-path", required=True)
    parser.add_argument("--global-bundle-path", required=True)
    parser.add_argument(
        "--feature-config",
        action="append",
    ...
    parser.add_argument("--output-dir", default="outputs/metrics/hybrid")
...
def main() -> int:
    ...
    if lambda_value is None:
        print("[hybrid-eval] selecting lambda on valid split")
        _, valid_metrics = evaluate_local_system(
            task=args.task,
            split="valid",
            ...
            global_bundle_path=args.global_bundle_path,
            top_k_pos=args.top_k_pos,
            top_k_neg=args.top_k_neg,
            strict_cross_scaffold_pairs=not args.allow_same_scaffold,
            lambda_value=None,
        )
        lambda_value = float(valid_metrics["lambda"])
        print(f"[hybrid-eval] selected lambda={lambda_value:.4f}")
