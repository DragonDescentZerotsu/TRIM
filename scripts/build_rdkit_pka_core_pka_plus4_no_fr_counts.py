...
CORE_PKA_PLUS4_COLUMNS = (
    ...
    "pka__most_basic_pka",
    "pka__acid_site_pka_sum",
    "pka__base_site_pka_sum",
    "pka__acid_site_pka_max",
    "pka__base_site_pka_min",
)
...
DEFAULT_OUTPUT = (
    "data/features/rdkit_descriptors_and_pka_easy_to_NLP_Lv1_core_pka_plus4_no_fr_counts/"
    "tdc_no_conflict_labels_salt_removed_unique_smiles_rdkit_descriptors_and_pka_easy_to_NLP_Lv1_core_pka_plus4_no_fr_counts.csv"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "and a compact pKa subset: the 7 core pKa features plus 4 key summary features."
        )
    )
...
def main() -> int:
    ...
    present_pka = [column for column in CORE_PKA_PLUS4_COLUMNS if column in frame.columns]
    missing_pka = [column for column in CORE_PKA_PLUS4_COLUMNS if column not in frame.columns]

    keep_columns = [
        column
        for column in frame.columns
        if column == "smiles" or not str(column).startswith("pka__") or column in present_pka
    ]
    ...
    if missing_pka:
        print(f"missing_pka_columns={missing_pka}")
    return 0
